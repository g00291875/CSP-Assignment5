...
public class TicTacToeServer extends JFrame {
   ...
   private class Player extends Thread {
      ...
      public void otherPlayerMoved( int location )  // where does this return to????
      {
         try {
            if (location == 10) {
               displayMessage("sending restart request to player [" +  playerNumber + "]\n");
               output.writeUTF("restart");
            }
            else if (location == 11) {
                           try {
               players[currentPlayer].output.writeUTF(whoIsWinner);
           } catch (IOException e) {
               e.printStackTrace();
           }
            }
                   ...
                   while (true) {
                       while (!isGameOver()) {          /***** MAIN LOOP MAIN LOOP MAIN LOOP MAIN LOOP MAIN LOOP **/
                           try {
                               ...
                               if (location != 10)
                                   displayMessage("\nreceived " + location + " from  player [" + currentPlayer + "]\n");
                               else
                                   displayMessage("\nreceived restart request from  player [" + currentPlayer + "]\n");
                               ...
                               if (validateAndMove(location, playerNumber)) { // returns true if it's a valid move
                                   ...
                                   if (location != 10) {
                                       ...
                                       displayMessage("sending location: " + location + " to player [" + currentPlayer + "]\n");
                                   } else {
                                       output.writeUTF("request ok");// this thread to other client
                                       displayMessage("sending 'REQUEST OK' back\n");
                                   }
                               } else
...
/********************************************************************************************************/
                       try {
                       if (validateAndMove(11, playerNumber)) {
...
/********************************************************************************************************/
                       //   synchronized (this) {
                           ...
                           while(reset == false) {
                               if (reset == false){
                                   players[(currentPlayer + 1) % 2].output.writeUTF("\n something");
                                   players[(currentPlayer + 1) % 2].output.writeUTF("\n something");
                                   reset = true;
                               }
                           }
